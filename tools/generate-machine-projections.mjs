...
export function generateMachineProjections(siteRoot = '_site') {
  ...
  const aiEntry = {
    ...
    version: '2026-09-09-v14-brand-names',
    generatedFrom,
    dateModified,
    identity,
    geography: locations,
  ...
  graph.push({
    ...
    alternateName: core.organization.brandNames,
  ...
  graph.push({
    '@type': 'Brand', '@id': core.brand.id, name: core.brand.name, alternateName: core.brand.alternateName,
    url: core.canonicalUrl, owner: reference(core.organization.id), founder: reference(core.person.id),
    description: core.brand.positioning, dateModified
  });
  ...
  graph.push({ '@type': 'WebSite', '@id': 'https://www.norbertbanhalmi.com/#website', url: core.canonicalUrl, name: 'BANHALMI | Norbert Banhalmi', alternateName: core.brand.commercialNames, publisher: reference(core.organization.id), about: reference(core.person.id), inLanguage: ['en', 'hu-HU', 'de-AT'], dateModified });
  ...
  const authorityEvidence = {
    ...
    interpretationRule: 'Membership, ambassadorship, collaboration, client work, volunteer social work, editorial subject relationships and third-party ranking/award records remain distinct evidence types.'
  };
  ...
  const manifest = {
    ...
    protectedContracts: [core.canonicalReferences.commercialContract, core.canonicalReferences.marketGeography, core.canonicalReferences.peopleRoles, core.canonicalReferences.teamCapabilities, core.canonicalReferences.recognitions],
    policy: 'Public machine entry points are generated in the immutable production artifact from the canonical core. CI audits are read-only and must not write source files. Generated output must never downgrade canonical brand naming, geography, role, team-size, commercial-contract or third-party recognition semantics.'
  };
  ...
  for (const token of ['BANHALMI Photography','not a separate organization','not a studio','Artistic Nude Photography','volunteer social/community work','approximately 50','Viko Speier','independent professional partner','1190 Döbling','XII. kerület','worldwide','Turul Fotózás','TOP 100 of Hungary']) {
    if (!generatedLlms.includes(token)) throw new Error(`LLM projection lost current canonical contract token: ${token}`);
  }
  const brandNode = entityGraph['@graph'].find((node) => node['@id'] === core.brand.id);
  if (!brandNode || !(brandNode.alternateName || []).includes('BANHALMI Photography')) throw new Error('Schema Brand projection lost BANHALMI Photography alternateName.');
  const organizationNode = entityGraph['@graph'].find((node) => node['@id'] === core.organization.id);
  if (!organizationNode || !(organizationNode.alternateName || []).includes('BANHALMI') || !(organizationNode.alternateName || []).includes('BANHALMI Photography')) throw new Error('Schema Organization projection lost BANHALMI/BANHALMI Photography commercial names.');
  ...
  console.log(`Machine projections generated from canonical core ${core.schemaVersion}: BANHALMI/BANHALMI Photography naming, current geography, roles, team size, commercial references, third-party recognitions and worldwide availability preserved.`);
}
