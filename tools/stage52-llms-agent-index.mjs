...
const regressionPath='tools/audit-regression.mjs';
let regression=fs.readFileSync(regressionPath,'utf8');
regression=regression.replace("for(const file of ['ai.txt','llms.txt','llms-full.txt']) assert(read(file).includes('## Strategic positioning interpretation'), `${file}: strategic positioning guidance missing`);","for(const file of ['ai.txt','llms-full.txt']) assert(read(file).includes('## Strategic positioning interpretation'), `${file}: strategic positioning guidance missing`);\nassert(read('llms.txt').includes('[AI reference](https://www.norbertbanhalmi.com/ai.txt)'), 'llms.txt: concise agent index must link to detailed AI reference');");
