...
function inspect(rel,abs){const h=fs.readFileSync(abs,'utf8');if(isRedirect(h))return;const main=h.match(/<main\b[\s\S]*?<\/main>/i)?.[0]||'';if(main.replace(/<[^>]+>/g,' ').replace(/\s+/g,' ').trim().length<160)return;const d=get(h,'description'),og=get(h,'og:description'),tw=get(h,'twitter:description');if(!d)failures.push(`${rel}: meta description missing`);if(!og)failures.push(`${rel}: og:description missing`);if(!tw)failures.push(`${rel}: twitter:description missing`);if(d&&(/(?:…|\.\.\.)\s*$/.test(d)))failures.push(`${rel}: artificially truncated meta description: ${d}`);if(d&&og&&d!==og)failures.push(`${rel}: meta/OG description drift`);if(d&&tw&&d!==tw)failures.push(`${rel}: meta/Twitter description drift`);if(d&&(d.length<65||d.length>190))failures.push(`${rel}: meta description length ${d.length} outside 65–190 character quality band`)}
walk('.');
if(failures.length){console.error('BANHALMI metadata quality audit FAILED:\n'+failures.map(x=>' - '+x).join('\n'));process.exit(1)}
console.log('BANHALMI metadata quality audit passed across all real content pages: complete, synchronized meta/OG/Twitter descriptions.');
