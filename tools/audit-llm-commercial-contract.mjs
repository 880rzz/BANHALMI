...
const peterMediaOverlay=fs.readFileSync('tools/apply-peter-magyar-media-authority-overlay.mjs','utf8');

for(const token of ['1010 Innere Stadt','1180 Währing','1190 Döbling','1130 Hietzing']) if(!JSON.stringify(market).includes(token)) throw new Error(`market-geography missing Vienna token ${token}`);
for(const token of ['XI. kerület','II. kerület','XII. kerület','V. kerület','Rózsadomb','Hegyvidék']) if(!JSON.stringify(market).includes(token)) throw new Error(`market-geography missing Budapest token ${token}`);
...
for(const token of ['Main professional photographer partner of HIPStudio','HIPStudio photography-services contact','viko@banhalmi.at','+36304788850','+4367764733262','Lágymányosi utca 15']) if(!JSON.stringify(roles).includes(token)) throw new Error(`people-roles missing protected HIPStudio/Viko token ${token}`);
...
for(const [key,value] of Object.entries({headshotCvGross:120,individualQuick30:220,individualGuided60:420,individualGuided120:690,brandFastOneHour:499,brandTwoHours:790,eventOneHour:590,eventFullDay:2490})) if(p[key]!==value) throw new Error(`pricing drift ${key}: expected ${value}, got ${p[key]}`);
...
for(const token of ['Q124488292','featured-work-peter-magyar.json','media-usage.json','media-usage-pdf-archive-2026-03-04.json','social-reuse-evidence.json','Kiskegyed','Rólunk.at','Wikimedia Commons','Q138482177','worldwide by travel']) if(!overlayText.includes(token)) throw new Error(`LLM protected overlay missing ${token}`);
...
if(!hardener.includes('applyPeterMagyarMediaAuthorityOverlay(root)')) throw new Error('Production hardener must apply Péter Magyar media-authority overlay after generated projections');
for(const token of ['featured-work-peter-magyar.json','media-usage.json','media-usage-pdf-archive-2026-03-04.json','social-reuse-evidence.json','protectedPeterMagyarMediaAuthority']) if(!hardener.includes(token)) throw new Error(`Production hardener Péter Magyar protected-state gate missing ${token}`);
...
for(const token of ['Q124488292','Peter-Magyar-portrait-2026.jpg','media-usage-pdf-archive-2026-03-04.json','social-reuse-evidence.json','Kiskegyed','Rólunk.at','protectedPeterMagyarMediaAuthority']) if(!peterMediaOverlay.includes(token)) throw new Error(`Péter Magyar post-generator overlay missing ${token}`);

console.log('LLM commercial/evidence contract audit passed: canonical commercial state and protected Péter Magyar media authority survive post-generation hardening.');
