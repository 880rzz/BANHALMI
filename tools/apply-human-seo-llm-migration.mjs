...
function replaceRequired(text, from, to, file) {
  if (!text.includes(from)) throw new Error(`${file}: expected source text was not found: ${from.slice(0, 90)}`);
...
const pages = {
  'about/index.html': {
    ...
    h1Old: 'I never planned to become a photographer. Then a camera found its way into my hands.',
    h1New: 'I did not set out to become a photographer. I began by documenting what was already in front of me.',
    leadOld: 'The first camera came into my hands during military service. Since then I have photographed leaders, artists, weddings, international brands and people going through some of the hardest periods of their lives. The settings changed. The question did not: how can a photograph show the person rather than merely record their appearance?',
  ...
  'hu/eletmu/index.html': {
    ...
    h1Old: 'Nem készültem fotósnak. Aztán a kezembe került egy fényképezőgép.',
  ...
  'de-at/werk/index.html': {
    ...
    h1Old: 'Ich wollte nie Fotograf werden. Dann hielt ich plötzlich eine Kamera in der Hand.',
    h1New: 'Ich wollte nicht Fotograf werden. Es begann damit, dass ich festhalten wollte, was vor mir geschah.',
    leadOld: 'Die erste Kamera kam während des Militärdienstes in meine Hände. Seitdem habe ich Führungskräfte, Künstler:innen, Hochzeiten, internationale Marken und Menschen in schwierigen Lebensphasen fotografiert. Die Situationen wechselten, die Frage blieb: Wie kann ein Bild den Menschen zeigen und nicht nur sein Äußeres festhalten?',
...
for (const [relative, copy] of Object.entries(pages)) {
  ...
  html = replaceRequired(html, copy.metaOld, copy.metaNew, relative);
  html = replaceRequired(html, copy.h1Old, copy.h1New, relative);
  html = replaceRequired(html, copy.originOld, copy.originNew, relative);
  html = replaceRequired(html, copy.timelineOld, copy.timelineNew, relative);
