...
for (const cfg of branchFiles) {
  ...
  const stepRegex = /(<div class="quote-step"><h3>2 · [^<]+<\/h3>)/;
  if (!stepRegex.test(html)) throw new Error(`${cfg.path}: step 2 marker not found`);
  html = html.replace(stepRegex, `$1${cfg.guide}`);
