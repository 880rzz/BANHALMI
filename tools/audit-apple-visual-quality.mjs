...
for(const width of widths){
 ...
 for(const pathname of pages){
  ...
  const result=await page.evaluate(()=>{
   ...
   for(const el of longText){const s=getComputedStyle(el),r=el.getBoundingClientRect();const fs=px(s.fontSize),lh=px(s.lineHeight)/(fs||1);const centeredAllowed=!!el.closest('.cta-band,.hero-centered,.centered,.site-footer,.error-page,.statement');if(s.textAlign==='justify')issues.push(`${name(el)} uses justified text`);if(s.textAlign!=='left'&&!centeredAllowed)issues.push(`${name(el)} long prose text-align=${s.textAlign}`);if(fs<16)issues.push(`${name(el)} long prose font-size ${fs.toFixed(1)}px < 16px`);if(lh<1.35||lh>1.82)issues.push(`${name(el)} long prose line-height ${lh.toFixed(2)}`);if(w>=1024&&r.width>860)issues.push(`${name(el)} long prose width ${r.width.toFixed(0)}px > 860px`);if(w<=768&&!el.closest('.gallery,.collage')&&(r.left<15||r.right>w-15))issues.push(`${name(el)} page gutter [${r.left.toFixed(1)},${(w-r.right).toFixed(1)}]px`);}
   for(const h of document.querySelectorAll('main h1,main h2,main h3,header h1')){if(!visible(h))continue;const s=getComputedStyle(h),r=h.getBoundingClientRect(),fs=px(s.fontSize),lh=px(s.lineHeight)/(fs||1);if(lh<0.98||lh>1.32)issues.push(`${name(h)} heading line-height ${lh.toFixed(2)}`);const tag=h.tagName.toLowerCase();const lim=tag==='h1'?(w<=430?[32,50]:w<=768?[34,58]:[38,70]):tag==='h2'?(w<=430?[24,42]:[24,48]):[18,34];if(fs<lim[0]||fs>lim[1])issues.push(`${name(h)} font-size ${fs.toFixed(1)}px outside ${lim[0]}–${lim[1]}px`);if(w<=768&&!h.closest('.gallery,.collage')&&(r.left<15||r.right>w-15))issues.push(`${name(h)} heading violates page gutter [${r.left.toFixed(1)},${r.right.toFixed(1)}]`);}
   // Full-bleed Apple section bands: any direct-main visible color surface spans the viewport, while inner content remains wrapped.
   for(const sec of document.querySelectorAll('main > section')){if(!visible(sec))continue;const r=sec.getBoundingClientRect(),s=getComputedStyle(sec);const bg=s.backgroundColor;const visiblyColored=bg!=='rgba(0, 0, 0, 0)'&&bg!==bodyBg;if(visiblyColored&&r.width<w-2)issues.push(`${name(sec)} colored section not full viewport (${r.width.toFixed(0)}/${w})`);if(s.contentVisibility==='auto')issues.push(`${name(sec)} content-visibility:auto can create blank bands`);}
...
console.log(`BANHALMI Apple visual quality audit passed: ${pages.length} pages × ${widths.length} viewports; typography, leading, alignment, reading measure, gutters, full-bleed sections, controls and cell geometry verified.`);
