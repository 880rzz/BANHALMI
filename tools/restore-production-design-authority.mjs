...
for(const required of [
  ...
  `@media(min-width:${Number(design.layout.footer?.desktopMinPx||1180)}px){html body .site-footer .footer-grid{grid-template-columns:minmax(220px,2fr) repeat(${Math.max(1,Number(design.layout.footer?.desktopColumns||6)-1)},minmax(0,1fr))!important`,
...
console.log(`BANHALMI production design compiled from ${design.version}; ${checked} HTML files checked, ${normalized} artifact HTML file(s) normalized, ${privateInjected} private-event quote adapter injection(s), ${pdfPatched} PDF label patch(es). Standard/structured canvases, ${Number(design.responsive?.touchTargetPx||44)}px header controls, text-only active navigation, compact responsive footer and normal document flow are active.`);
