...
for(const required of [
  ...
  'a[aria-current="page"],a.active,.active>a',
  'min-height:0!important;height:auto!important;flex:none!important',
  'left-aligned semantic block shares the same optical start',
  '.faq-topic>:is(h2,h3,p,.lead,.description)'
...
console.log(`BANHALMI production design compiled from ${design.version}; ${checked} HTML files checked, ${normalized} artifact HTML file(s) normalized, ${privateInjected} private-event quote adapter injection(s), ${pdfPatched} PDF label patch(es). Standard and structured canvases, canonical content-axis including nested FAQ topics, text-only active navigation, content-height footer and card rhythm projections are active.`);
