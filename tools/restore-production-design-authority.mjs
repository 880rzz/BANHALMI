...
console.log(`BANHALMI production design compiled from ${design.version}; ${checked} HTML files checked, ${normalized} artifact HTML file(s) normalized, ${privateInjected} private-event quote adapter injection(s), ${pdfPatched} PDF label patch(es). Standard/structured canvases, ${Number(design.responsive?.touchTargetPx||44)}px header controls, text-only active navigation and normal footer flow are active.`);
