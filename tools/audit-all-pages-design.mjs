...
for(const width of widths){
  ...
  for(const file of contentFiles){
    ...
    const r=await page.evaluate(({pageMaxPx,structuredMaxPx,structuredBreakpointPx,structuredSelector})=>{
      ...
      const touch=[];if(innerWidth<=1024){for(const el of document.querySelectorAll('button,summary,.btn,.menu-btn,.nav-cta,input:not([type="hidden"]):not([type="checkbox"]):not([type="radio"]),select,textarea')){if(!visible(el))continue;const b=el.getBoundingClientRect();if(b.height<43.5)touch.push(`${el.tagName.toLowerCase()} height=${b.height.toFixed(1)}`);if((el.matches('button,.menu-btn')||el.getAttribute('role')==='button')&&b.width<43.5)touch.push(`${el.tagName.toLowerCase()} width=${b.width.toFixed(1)}`);if(touch.length>=8)break;}}
    ...
    if(r.footerHeight>height*0.85&&width>=768)failures.push(`${rel} @${width}x${height}: footer occupies ${(r.footerHeight/height*100).toFixed(0)}% of viewport`);
...
console.log(`BANHALMI exhaustive design audit passed: ${contentFiles.length} content pages × ${widths.length} device-class viewports = ${checks} checks from 320×568 through 3840×2160; canonical canvases, overflow, media, centering, footer flow, active navigation and touch geometry verified.`);
