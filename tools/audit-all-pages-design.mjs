...
for(const width of widths){
  ...
  for(const file of contentFiles){
    ...
    const r=await page.evaluate(({pageMaxPx})=>{
      ...
      for(const w of document.querySelectorAll('main .wrap')){
        ...
        if(b.width>Math.min(innerWidth,pageMaxPx)+4)wraps.push(b.width);
      }
    ...
    for(const w of r.wraps)failures.push(`${rel} @${width}: .wrap exceeds canonical page max ${pageMaxPx}px (${w.toFixed(1)}px)`);
...
console.log(`BANHALMI exhaustive design audit passed: ${contentFiles.length} content pages × ${widths.length} viewports = ${checks} render checks; canonical page max ${pageMaxPx}px, document overflow, shell containment, surfaces and quote controls verified against the approved visual baseline.`);
