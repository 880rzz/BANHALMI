...
for(const token of ['Services','Work','About · Pricing · Contact','Executive Portraits','Selected Work','Art Archive','Szolgáltatások','Munkák','Rólam · Árak · Kapcsolat','Executive portré','Válogatott munkák','Művészeti archívum','Leistungen','Arbeiten','Über BANHALMI · Preise · Kontakt','Executive-Porträts','Ausgewählte Arbeiten','Kunstarchiv',"grid.append(svc,main,foot)"])if(!menu.includes(token))failures.push('service-first multilingual navigation token missing: '+token);
for(const retired of ["['/about/','Oeuvre'","['/hu/eletmu/','Életmű'","['/de-at/werk/','Werk'"])if(menu.includes(retired))failures.push('retired primary navigation structure returned: '+retired);
if(failures.length){console.error('Navigation/CTA contract FAILED:\n'+failures.map(x=>' - '+x).join('\n'));process.exit(1)}
console.log('Navigation/CTA contract passed: canonical routes, descriptive CTAs and service-first EN/HU/DE navigation hierarchy are release-blocking.');
