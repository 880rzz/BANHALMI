...
// Owner-reported regressions were visible at compact tablet/mobile widths and on
// genuinely wide desktop canvases. Keep the canonical matrix and add 1920px so
// a 1200px-centred desktop column cannot be mistaken for a fully-used canvas.
  ...
  for(const pathname of pages){
    ...
    const issues=await page.evaluate(()=>{
      ...
      for(const el of document.querySelectorAll('main section')){
        ...
        // Short factual/list sections should not inherit the same vertical air as
        // a hero or major editorial chapter.
        const compact=text.length<700&&media===0;
        const padLimit=compact?(w>=1280?88:104):112;
      ...
      for(const h of document.querySelectorAll('main h1,main h2,main h3,header h1')){
        ...
        if(fs<1)continue;
        let min=0,max=999;
        if(h.matches('h1')){min=w<=430?34:w<=768?38:40;max=w<=430?42:w<=768?46:58}
        else if(h.matches('h2')){min=w<=430?24:w<=768?25:26;max=w<=430?32:w<=768?34:38}
        else {min=16;max=28}
      ...
      for(const h of document.querySelectorAll('main h1,main h2,main h3')){
        ...
        const hr=h.getBoundingClientRect(),nr=n.getBoundingClientRect(),gap=nr.top-hr.bottom;const min=10,max=h.matches('h1')?44:h.matches('h2')?30:26;if(gap<min||gap>max)out.push(`${name(h)} → ${name(n)} gap ${gap.toFixed(0)}px outside ${min}-${max}px`)
      }

      // Shared editorial introductions must start on one optical axis. A centred
      // component is allowed only when the component explicitly declares that
      // semantic role; accidental text-align:center is a regression.
      ...
      // Quote cards: the radio column, text column and info control must remain
      // distinct. This guards the exact overlap shown in the owner's mobile
      // screenshots, where long titles ran back over the radio control.
      ...
      // Odd membership counts must compose deliberately on two-column mobile
      // grids instead of leaving a visually accidental empty final cell.
      ...
      // Wide desktop must use the available canvas for structured content. Long
      // prose can stay narrow; grids, partner evidence and quote workspaces may not.
      if(w>=1440){for(const block of document.querySelectorAll('main .cards,main .partner-grid,main .partner-grid-memberships,main .grid-3,main .grid-4,main .steps,main .smart-quote-layout')){if(!visible(block))continue;const r=block.getBoundingClientRect();const required=Math.min(w*.76,1380);if(r.width<required)out.push(`${name(block)} desktop canvas ${r.width.toFixed(0)}px < ${required.toFixed(0)}px at ${w}px`)}}
