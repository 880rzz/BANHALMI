...
(function(){
  ...
  function showAgeDialog(index,trigger){pendingIndex=index;lastTrigger=trigger||null;ageDialog.classList.add('open');ageDialog.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';ageDialog.querySelector('[data-age-accept]').focus();}
  function closeAgeDialog(restore){ageDialog.classList.remove('open');ageDialog.setAttribute('aria-hidden','true');document.body.style.overflow='';if(restore&&lastTrigger)lastTrigger.focus();}
  ...
  function openAt(index){collect();if(!items.length)return;current=(index+items.length)%items.length;var el=items[current];img.src=el.getAttribute('data-lightbox-src');img.alt=el.getAttribute('data-lightbox-cap')||'';cap.textContent=el.getAttribute('data-lightbox-cap')||'';linkBox.innerHTML='';for(var n=1;n<=12;n++){var href=el.getAttribute('data-lightbox-link-'+n);var label=el.getAttribute('data-lightbox-link-'+n+'-label');if(href&&label){var a=document.createElement('a');a.href=href;if(/^https?:\/\//i.test(href)){a.target='_blank';a.rel='noopener noreferrer';}a.textContent=label+' ↗';linkBox.appendChild(a);}}linkBox.hidden=!linkBox.children.length;lb.classList.add('open');lb.setAttribute('aria-hidden','false');document.body.style.overflow='hidden';lb.querySelector('.universal-lightbox-close').focus();}
  function close(){lb.classList.remove('open');lb.setAttribute('aria-hidden','true');document.body.style.overflow='';img.removeAttribute('src');if(lastTrigger)lastTrigger.focus();}
